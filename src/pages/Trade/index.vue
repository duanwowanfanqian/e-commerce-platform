<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressInfo" :key="address.id" @click="selectDefault(address,addressInfo)">
...
export default {
  ...
  watch: {
    // 因为服务器给的数据中没有默认收件人,所以需要我们自己设置(后续如果服务器给了默认收件人就可以将监听删除);我们监听addressInfo变化.
    // 当其发生变化时(即拿到服务器数据时)将第一的收件人信息设置为默认收件人,因为我们不是深度监听,所以只要不重新指向新数组,函数就不会触发.
    addressInfo () {
      this.$store.getters.defaultMessage.isDefault = 1;
    }
  }
}
</script>
