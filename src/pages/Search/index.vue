...
export default {
  ...
  beforeMount () {
    ...
    // ...... 
    */
    // Object.assign方法将所有可枚举和自有属性从一个或多个源对象（第一个以后的参数）复制到目标对象（第一个参数），返回修改后的对象(会修改目标对象，也会返回新对象)。
  ...
  methods: {
    ...
    removeCategoryName () {
      ...
      // 当参数属性值为空时带给服务器参数说明可有可无，如果属性值为空的字符串还是会把相应的字段带给服务器
      // 但是你把啊相应字段变更为undefined，当前这字段不会带给服务器
  ...
  watch: {
    ...
    $route () {
      // 每一次请求完毕，应该把对应的1，2，3级分类的id置空，让他接受下一次的相应等级的分类id
