        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转，只是简单的从A路由跳转到B路由，而这里的路由跳转，需要先将数据发送到服务器中，然后进行跳转 -->
...
export default {
  ...
  methods: {
    ...
    changeSkuNum (e) {
      ...
      // 不正常情况，输入的为菲数值，或小于一的数。 遇到这种问题我们将skuNum转换为一
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        // 为了防止正常中的不正常情况：即输入的数是小数,所以再赋值之前将其转换为整数
    ...
    async addShopCar () {
      ...
      try {
        // 1:发请求---将产品加入到数据库(通知服务器),在async函数下会先执行await后面的操作
